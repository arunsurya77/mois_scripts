...
t_arr= np.linspace(20,295,10)
wave_arr=np.array([2.0300,2.09800,2.2000,2.3020,2.3700])
wave_arr=np.linspace(0.5,3,1000)	
table=np.zeros([len(wave_arr),2])
table[:,0]=wave_arr
T=120
...
print("BAF2")
for k in range(len(wave_arr)):
	wave=wave_arr[k]
	n=np.sqrt(1+((BAF2_S1*(wave**2))/((wave**2)-BAF2_L1))+((BAF2_S2*(wave**2))/((wave**2)-BAF2_L2))+((BAF2_S3*(wave**2))/((wave**2)-BAF2_L3)))
	print(k,wave,n)
	table[k,1]=n
print(" ")	
np.savetxt('BAF2_'+str(T)+'K.DAT',table)

print("ZNS")
for k in range(len(wave_arr)):
	wave=wave_arr[k]
	n=np.sqrt(1+((ZNS_S1*(wave**2))/((wave**2)-ZNS_L1))+((ZNS_S2*(wave**2))/((wave**2)-ZNS_L2))+((ZNS_S3*(wave**2))/((wave**2)-ZNS_L3)))
	print(k,wave,n)
	table[k,1]=n
print(" ")	
np.savetxt('ZNS_'+str(T)+'K.DAT',table)

print("CAF2")
for k in range(len(wave_arr)):
	wave=wave_arr[k]
	n=np.sqrt(1+((CAF2_S1*(wave**2))/((wave**2)-CAF2_L1))+((CAF2_S2*(wave**2))/((wave**2)-CAF2_L2))+((CAF2_S3*(wave**2))/((wave**2)-CAF2_L3)))
	print(k,wave,n)
print(" ")	
np.savetxt('CAF2_'+str(T)+'K.DAT',table)

print("ZNSE")
for k in range(len(wave_arr)):
	wave=wave_arr[k]
	n=np.sqrt(1+((ZNSE_S1*(wave**2))/((wave**2)-ZNSE_L1))+((ZNSE_S2*(wave**2))/((wave**2)-ZNSE_L2))+((ZNSE_S3*(wave**2))/((wave**2)-ZNSE_L3)))
	print(k,wave,n)
	table[k,1]=n
print(" ")	
np.savetxt('ZNSE_'+str(T)+'K.DAT',table)

print("FSIL")
for k in range(len(wave_arr)):
	wave=wave_arr[k]
	n=np.sqrt(1+((FSIL_S1*(wave**2))/((wave**2)-FSIL_L1))+((FSIL_S2*(wave**2))/((wave**2)-FSIL_L2))+((FSIL_S3*(wave**2))/((wave**2)-FSIL_L3)))
	print(k,wave,n)
	table[k,1]=n
print(" ")	
np.savetxt('FSIL_'+str(T)+'K.DAT',table)
